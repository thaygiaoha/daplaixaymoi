...
const handleAuth = async (e: React.FormEvent) => {
  ...
  const fetchSubjects = async () => {
    try {
      // THAY ĐỔI: Sử dụng link App Script của bạn (link có chữ /exec)
      const scriptUrl = 'https://script.google.com/macros/s/AKfycbwZdb9kvZk6acp9aVkZBvsV-4hXpnQOdHYzOS1jHFmvwPsYcjXz7IDBQ8xuF_PgB3Bkkg/exec?action=getSubjects';

      const response = await fetch(scriptUrl);
      const jsonData = await response.json(); // Lấy dữ liệu dạng JSON trực tiếp

      // jsonData lúc này có dạng: [["Môn", "Cấp", "Chọn môn", "Link"], ["Toán", "Lớp 6", ...]]
      // Chúng ta bỏ hàng đầu tiên (header) bằng .slice(1)
      const rows = jsonData.slice(1);

      const parsedData = rows.map(cols => {
        return {
          mon: cols[0] ? cols[0].toString().trim() : "",
          caphoc: cols[1] ? cols[1].toString().trim() : "",
          chonmon: cols[2] ? cols[2].toString().trim() : "",
          link: cols[3] ? cols[3].toString().trim() : ""
        };
      }).filter(item => item.mon && item.mon !== "");
      ...
      // Lấy danh sách Cấp học duy nhất
      const caps = [...new Set(parsedData.map(item => item.caphoc))].filter(Boolean);
      setDynamicLevels(caps);

      console.log("Dữ liệu môn học từ Script:", parsedData);
    } catch (error) {
      console.error("Lỗi lấy dữ liệu qua App Script:", error);
    }
  };
