...
const RESET_RESULT_PASSWORD = "6688@"; // mật khẩu xóa KQ
...
function getExamsList() {
  ...
  if (!sheet) {
    return createResponse("error", "Không tìm thấy sheet ketqua");
  }
...
// Reset kết quả
function resetKetQua(password, mode, exams) {

  if (password !== RESET_RESULT_PASSWORD) {
  ...
  if (mode === "all") {
    ...
    return createResponse("success", "Đã xóa toàn bộ kết quả");
  }
  ...
  if (mode === "byExams") {
    ...
    data.forEach((row, index) => {
      ...
      const rowExams = row[1]; // CỘT B = index 1
    ...
    return createResponse("success", "Đã xóa theo mã exams");
  }
